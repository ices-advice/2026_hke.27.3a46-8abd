################################################################################
#  hke.27.3a46-8abd_assessment : short-term forecast GM low runs               #
#------------------------------------------------------------------------------#
...
# model_02b_Forecast_runs.R - STF runs (with geometric mean of low last recruitments)
...
# load(file.path(stfGMlowlast_path, "info_models", "fmult.RData")) # fmult, Fmult_names
...
# create forecast folder and subfolders (if the first time)
  ...
  dir.runN.new <- stfGMlowlast_path
  ...
  for(j in 1:length(tacs)){
    ...
    file.copy(paste(dir.forecastTAC,  paste0("forecastGMlowlast_",tacs[j], ".ss") , sep="/"),
...
forecastModels <- SSgetoutput(dirvec=file.path(stfGMlowlast_path, Fmult_names))
...
for (sc in sc.prob) {
  ...
  file.copy(paste(dir.forecastTAC,  paste0("forecastGMlowlast_",tacs[sc], ".ss") , sep="/"),
